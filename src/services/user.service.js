...
const createUser = async (userBody) => {
  ...
  if (await User.isAccountNumber(userBody.accountNumber)) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Account Number already taken');
  }
  if (await User.isIdentityNumber(userBody.identityNumber)) {
...
const updateUserById = async (userId, updateBody) => {
  ...
  if (updateBody.accountNumber && (await User.isAccountNumber(updateBody.accountNumber, userId))) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Account Number already taken');
  }
  if (updateBody.identityNumber && (await User.isIdentityNumber(updateBody.identityNumber, userId))) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Identity Number already taken');
  }
